...
где $Z_i(i = 1, 2, ..., k)$ имеет нормальное распределение $N(0; 1)$.
...
Распределение хи-квадрат является частным случаем гамма-распределениея с гамма-параметрами $a = \frac{df}{2}$, $loc = 0$ и $scale = 2$.
...
- Диспресия: $D(X) = 2*k$
- Мода: $Mo(X) = 0 \text { для } k < 2, Mo(X) = k - 2, \text { если } X \geq 2$
- Коэффициент асимметрии: $\sqrt{\frac{8}{k}}$
- Коэффициент эксцесса: $\frac{12}{k}$
- Медиана: примерно $\frac{k - 2}{3}$
